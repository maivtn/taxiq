...
function boot(query='?tab=estimate'){
 const dom=new JSDOM(readFileSync(new URL('./pos-front-desk.html', SOURCE_DIR),'utf8'),{url:'https://example.test/pages/pos-front-desk.html'+query,runScripts:'outside-only'}),w=dom.window;
 ...
 for(const name of ['salon-data','appointment-tickets','appointments-store','pos-turn-settings','team-calendar-content','pos-front-desk','service-assignments','front-desk-assignments','pos-estimate','front-desk-estimate']){
  const file=new URL('../assets/'+name+'.js', SOURCE_DIR);if(existsSync(file))w.eval(readFileSync(file,'utf8'));
 }
 return dom;
}
...
test('Front Desk Tickets shows guests checked in from Estimate',()=>{
 ...
 const queue=new JSDOM(readFileSync(new URL('./pos-front-desk-tickets.html', SOURCE_DIR),'utf8'),{url:'https://example.test/pages/pos-front-desk-tickets.html',runScripts:'outside-only'});
 ...
 for(const name of ['salon-data','appointment-tickets','appointments-store','ticket-workspace','pos-front-desk-tickets'])queue.window.eval(readFileSync(new URL('../assets/'+name+'.js', SOURCE_DIR),'utf8'));
