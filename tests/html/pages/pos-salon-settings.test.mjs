...
test('Welcome SMS Setup live-updates its preview on edit, template change and token insert',()=>{
 ...
 assert.deepEqual(templates.map(button=>button.dataset.templateKey),['new','returning']);
 assert.equal(templates.find(button=>button.dataset.templateKey==='returning').getAttribute('aria-pressed'),'true');
 ...
 templates.find(button=>button.dataset.templateKey==='new').click();
 assert.match(textarea.value,/welcome to \[Salon Name\]/);
