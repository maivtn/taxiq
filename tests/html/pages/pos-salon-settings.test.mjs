...
test('Welcome SMS Setup live-updates its preview on edit, template change and token insert',()=>{
 ...
 assert.deepEqual(templates.map(button=>button.dataset.templateKey),['welcome','simple-welcome']);
 assert.deepEqual(templates.map(button=>button.querySelector('.sms-template-card-title').textContent),['Welcome message','Simple welcome']);
 assert.equal(templates.some(button=>/check[ -]?in/i.test(button.textContent)),false);
 ...
 templates.find(button=>button.dataset.templateKey==='simple-welcome').click();
 assert.match(textarea.value,/glad you’re here/);
 assert.match(preview.textContent,/Welcome to Bitcoin Nail Bar, Sarah!/);
...
test('Wait Care tab owns its delay rules and complete SMS workflow',()=>{
 ...
 assert.deepEqual(templates.map(button=>button.dataset.templateKey),['delay-update','care-benefit','comfort-check-in','visit-preparation']);
