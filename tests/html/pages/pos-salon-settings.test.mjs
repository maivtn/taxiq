...
test('Welcome SMS Setup live-updates its preview on edit, template change and token insert',()=>{
 ...
 assert.deepEqual(templates.map(button=>button.dataset.templateKey),['new','returning','birthday']);
 ...
 templates.find(button=>button.dataset.templateKey==='birthday').click();
