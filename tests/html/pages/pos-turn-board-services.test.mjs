...
function boot(t, storage = {}) {
  ...
  const dom = new JSDOM(readFileSync(new URL('./pos-front-desk-turn-board.html', SOURCE_DIR), 'utf8'), {
  ...
  w.eval(['salon-data','pos-turn-settings','pos-front-desk-turn-board','pos-turn-board-services','pos-turn-board-tools']
    .map(name => readFileSync(new URL('../assets/' + name + '.js', SOURCE_DIR),'utf8')).join('\n'));
