...
function boot(saved,catalog,url='https://example.test/pages/pos-front-desk-tickets.html'){
 const dom=new JSDOM(readFileSync(new URL('./pos-front-desk-tickets.html', SOURCE_DIR),'utf8'),{url,runScripts:'outside-only'}),w=dom.window;
 ...
 for(const name of ['salon-data','appointment-tickets','appointments-store','ticket-workspace','pos-front-desk-tickets','pos-front-desk-overview'])w.eval(readFileSync(new URL('../assets/'+name+'.js', SOURCE_DIR),'utf8'));
 return dom;
}
...
for(const seeded of [false,true])test(`Front Desk services have catalog prices before splitting (menu seeded: ${seeded})`,()=>{
 let catalog;
 if(seeded){
  const salon=require('../../../html/assets/salon-data.js'),menu=require('../../../html/assets/appointment-service-catalog.js').normalize(require('../../../html/menu/menu.json'));
