...
const css = fs.readFileSync(path.join(SOURCE_DIR, '..', 'assets', 'pos-booking.css'), 'utf8');
const source = html + '\n' + runtime;
...
test('POS keeps the shared mode and tab activation contracts', () => {
  assert.match(source, /var TABS = \['checkin', 'todaybooking', 'tickets', 'booking', 'estimate', 'customers', 'clock', 'management', 'printer'\]/);
