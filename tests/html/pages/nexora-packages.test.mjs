...
const PAGE_URL = new URL('./nexora-packages.html', SOURCE_DIR);
const BOOKING_PAGE_URL = new URL('./booking-book-phase-1.html', SOURCE_DIR);
const SHELL_CSS_URL = new URL('../assets/nexora-shell.css', SOURCE_DIR);
const PACKAGE_CSS_URL = new URL('../assets/nexora-packages.css', SOURCE_DIR);
const PACKAGE_BILLING_DATA_URL = new URL('../assets/nexora-package-billing-data.js', SOURCE_DIR);
const PACKAGE_JS_URL = new URL('../assets/nexora-packages.js', SOURCE_DIR);
...
test('adds the package heading and ordered management tabs', () => {
  ...
  assert.doesNotMatch(historyTab, /Billing History|Purchase History/);
...
test('renders package history billing columns, statuses, and record actions', () => {
  ...
  assert.match(historyHTML, /<caption class="visually-hidden">Package history<\/caption>/);
  assert.doesNotMatch(historyHTML, /Billing history|Package purchase history/);
