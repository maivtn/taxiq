...
const page = readFileSync(new URL('./booking-book-phase-1.html', SOURCE_DIR), 'utf8');
const runtimeUrl = new URL('../assets/booking-settings-tabs.js', SOURCE_DIR);
...
function loadFeature(hash = '', query = '?tab=settings', beforeInitialize = () => {}) {
  ...
  styles.textContent = readFileSync(new URL('../assets/booking-settings-tabs.css', SOURCE_DIR), 'utf8');
...
test('keeps the original voice controls and pending upload state when navigating between sections', async (t) => {
  ...
  const { dom, window, document } = loadFeature('', '?tab=settings', ({ window, document }) => {
    ...
    window.eval(readFileSync(new URL('../assets/booking-knowledge.js', SOURCE_DIR), 'utf8'));
