...
const asset=name=>new URL('../assets/'+name+'.js', SOURCE_DIR);
function boot(){
...
test('Front Desk sends a service and Work Orders opens, accepts, starts, completes and synchronizes progress',async()=>{
 const fd=new JSDOM(readFileSync(new URL('./pos-front-desk.html', SOURCE_DIR),'utf8'),{url:'https://demo.test/pages/pos-front-desk.html?section=assign&ticket=jojo&view=table',runScripts:'outside-only'});
 ...
 const errors=[];
 const staff=new JSDOM(readFileSync(new URL('./staff-work-orders.html', SOURCE_DIR),'utf8'),{url:'https://demo.test/pages/staff-work-orders.html?technician=kayla',runScripts:'dangerously',beforeParse(w){
  w.scrollTo=()=>{};
...
test('assignment and checkout have independent reloadable ticket URLs and link back to the original Tickets page',()=>{
 const html=readFileSync(new URL('./pos-front-desk.html', SOURCE_DIR),'utf8');
...
test('Work Orders keeps usable demo tickets when assignment storage loads and refreshes',()=>{
 const errors=[];
 const dom=new JSDOM(readFileSync(new URL('./staff-work-orders.html', SOURCE_DIR),'utf8'),{
