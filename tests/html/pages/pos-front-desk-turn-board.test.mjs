...
function boot(){
 ...
 const dom=new JSDOM(readFileSync(new URL('./pos-front-desk-turn-board.html', SOURCE_DIR),'utf8'),{url:'https://example.test/pages/pos-front-desk-turn-board.html',runScripts:'dangerously',virtualConsole:vc,beforeParse(w){w.structuredClone=structuredClone;}});
 dom.window.eval(['salon-data','pos-turn-settings','pos-front-desk-turn-board','pos-turn-board-services','pos-turn-board-tools'].map(name=>readFileSync(new URL('../assets/'+name+'.js', SOURCE_DIR),'utf8')).join('\n'));
