...
function boot(saved){
 const dom=new JSDOM(fs.readFileSync(path.join(SOURCE_DIR,'pos-phase-1.html'),'utf8'),{url:'https://example.test/pages/pos-phase-1.html?tab=estimate',runScripts:'outside-only'}),w=dom.window;
 ...
 for(const script of w.document.scripts){
  if(script.src){const src=script.getAttribute('src');if(src.startsWith('../assets/'))w.eval(fs.readFileSync(path.join(SOURCE_DIR,src),'utf8'));}
