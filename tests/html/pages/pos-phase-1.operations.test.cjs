const SOURCE_DIR = require('node:path').resolve(__dirname, '../../../html/pages');
...
const html = fs.readFileSync(path.join(SOURCE_DIR, 'pos-phase-1.html'), 'utf8');
const phase2Html = fs.readFileSync(path.join(SOURCE_DIR, 'pos-phase-2.html'), 'utf8');
const bookingCss = fs.readFileSync(path.join(SOURCE_DIR, '..', 'assets', 'pos-booking.css'), 'utf8');
...
test('POS splits the legacy Operations/dispatch tab while temporarily hiding Check-in', () => {
  ...
  assert.doesNotMatch(html, /data-pos-tab="tickets"[^>]*>[\s\S]{0,140}Điều phối/);
  assert.doesNotMatch(html, /data-pos-tab="tickets"[^>]*>[\s\S]{0,100}Waiting List/);
...
test('Management exposes Services as table/card views with modal CRUD detail fields', () => {
  ...
  const categoryManagerRuntime = html.match(/function serviceCategoryManagerHtml\(categoryRows\)[\s\S]*?function serviceModalSummaryHtml\(service, mode\)/)?.[0] || '';
  ...
  assert.match(html, /data-mg-category-manager-toggle/);
  assert.match(html, /var serviceCategoryManagerCollapsed = true;/);
  assert.match(categoryManagerRuntime, /aria-expanded="' \+ \(!serviceCategoryManagerCollapsed \? 'true' : 'false'\)/);
  assert.match(categoryManagerRuntime, /serviceCategoryManagerCollapsed \? '' :/);
  assert.match(categoryManagerRuntime, /data-mg-category-table/);
  assert.match(html, /data-mg-category-add/);
  assert.match(html, /data-mg-category-modal/);
  assert.match(managementPanel, /class="sms-modal service-editor-modal" data-mg-category-modal/);
  assert.match(managementPanel, /data-mg-category-modal-title/);
  assert.match(managementPanel, /data-mg-category-form-shell/);
  assert.match(managementPanel, /data-mg-category-detail/);
  assert.match(managementPanel, /data-mg-category-form-name/);
  assert.match(managementPanel, /data-mg-category-form-kind/);
  assert.match(managementPanel, /data-mg-category-form-active/);
  assert.match(managementPanel, /data-mg-category-save-label/);
  ...
  assert.match(html, /function closeCategoryModal\(\)/);
  assert.match(html, /function readCategoryModal\(\)/);
  assert.match(html, /function saveCategoryModal\(\)/);
  ...
  assert.match(html, /var categoryManagerToggle = e\.target\.closest\('\[data-mg-category-manager-toggle\]'\);/);
  assert.match(html, /var categoryAdd = e\.target\.closest\('\[data-mg-category-add\]'\);/);
  assert.match(html, /openCategoryModal\(null, 'add'\);/);
  assert.match(html, /var categoryView = e\.target\.closest\('\[data-mg-category-view\]'\);/);
  assert.match(html, /openCategoryModal\(categoryView\.getAttribute\('data-mg-category-view'\), 'view'\);/);
  assert.match(html, /var categoryEdit = e\.target\.closest\('\[data-mg-category-edit\]'\);/);
  assert.match(html, /openCategoryModal\(categoryEdit\.getAttribute\('data-mg-category-edit'\), 'edit'\);/);
  assert.match(html, /if \(e\.target\.closest\('\[data-mg-category-save\]'\)\) \{ saveCategoryModal\(\); return; \}/);
  assert.match(categoryModalRuntime, /detailEl\.innerHTML = isReadOnly \? categoryDetailHtml\(category\) : '';/);
  assert.match(categoryModalRuntime, /formShellEl\.hidden = isReadOnly;/);
  assert.match(categoryModalRuntime, /saveLabelEl\.textContent = categoryModalMode === 'add' \? 'Create category' : 'Save changes';/);
