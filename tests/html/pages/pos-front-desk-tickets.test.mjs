...
function boot(url='https://example.test/pages/pos-front-desk-tickets.html'){
 const dom=new JSDOM(readFileSync(new URL('./pos-front-desk-tickets.html', SOURCE_DIR),'utf8'),{url,runScripts:'outside-only'});
 dom.window.HTMLDialogElement.prototype.showModal=function(){this.open=true;};dom.window.HTMLDialogElement.prototype.close=function(){this.open=false;};
 for(const name of ['salon-data','ticket-workspace','pos-front-desk-tickets'])dom.window.eval(readFileSync(new URL('../assets/'+name+'.js', SOURCE_DIR),'utf8'));
 dom.window.eval(readFileSync(new URL('../assets/pos-front-desk-overview.js', SOURCE_DIR),'utf8'));return dom;
}
