...
| Term | Definition |
| :--- | :--- |
| Waitlist SMS | Nhóm mẫu nhắc khách quay lại hoặc thông báo đã đến lượt. |
...
| Wait Care | Cập nhật việc phải chờ thêm và thời gian chờ dự kiến; không tự cấp ưu đãi. |
...
| :--- | :--- | :--- | :--- | :--- |
| 1 | Owner | Mở SMS Settings → Waitlist SMS hoặc Wait Care | Hiện mẫu đang lưu và phần xem trước | Wait Care vẫn là tab riêng. |
...
| 4 | Owner | Bấm Save Templates hoặc Save Wait Care Settings | Kiểm tra và lưu mẫu | Không gửi SMS cho khách khi lưu. |
...
| 2 | Lễ tân | Chọn Return Soon, Ready Now hoặc Wait Care | Nạp mẫu Owner đã lưu, điền thông tin của khách và lượt ghé | Wait Care chỉ xuất hiện khi được bật. |
...
- Là Owner, tôi muốn bật/tắt Wait Care để quyết định lễ tân có sử dụng nhóm tin này hay không. Bật chỉ cho phép sử dụng mẫu, không kích hoạt gửi tự động.
...
| Waitlist SMS | Giữ Return Soon và Ready Now; mỗi nhóm 2 Quick templates, Insert, ô sửa nội dung, Preview và Send Test. Một nút **Save Templates** lưu cả hai nhóm. |
| Wait Care | Giữ tab riêng, công tắc bật/tắt ở trên cùng, 2 mẫu delay, ô sửa, Preview, Send Test và một nút lưu. Bỏ Send mode và Waiting time since check-in. Không đưa lại Wait Care rules. |
| Insert | Return Soon và Ready Now dùng Customer name, Shop name, Visit link. Wait Care bổ sung Wait time. Bỏ Return notice; không thêm biến cấp ưu đãi trong phạm vi tin cập nhật chờ này. |
...
| Wait Care | Delay update | Hi [Customer Name], sorry for the wait at [Salon Name]. Estimated wait: [Wait Time]. Updates: [OneQR Link] |
| Wait Care | Wait time update | Hi [Customer Name], thanks for your patience at [Salon Name]. Estimated wait: [Wait Time]. Updates: [OneQR Link] |
...
6. Wait Care là tin cập nhật việc chờ; không tự cộng Points, tạo voucher hay cấp Benefit. Thao tác ưu đãi thuộc luồng riêng.
...
| Wait Care bị tắt | Không cho chọn/gửi Wait Care; Return Soon và Ready Now vẫn sử dụng được | Owner quản lý cấu hình. |

**Tiêu chí nghiệm thu chính:**

- Waitlist SMS không còn các trường timing, lựa chọn cách gửi hoặc nút Pause/Resume automation.
- Wait Care không còn mốc kích hoạt tự động; công tắc chỉ kiểm soát việc sử dụng nhóm mẫu.
- Mỗi loại có 2 mẫu gợi ý; chọn mẫu và Insert cập nhật đúng nội dung và Preview.
...
**Bật Wait Care có tự nhắn khi khách chờ lâu không?**

Không. Bật chỉ cho phép lễ tân sử dụng nhóm tin Wait Care.
