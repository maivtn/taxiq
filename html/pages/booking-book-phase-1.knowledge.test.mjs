...
const RUNTIME_URL = new URL('../assets/booking-knowledge.js', import.meta.url);
...
async function loadFeature(t, { holdProcessing = false, samples = false } = {}) {
  ...
  window.fetch = async (resource) => {
    ...
    const file = new URL(`./assets/ai-knowledge/${request.pathname.split('/').at(-1)}`, import.meta.url);
    return {
      ok: true,
      status: 200,
...
test('loads three real sample documents and downloads their complete source bytes', async (t) => {
  ...
  for (const row of root.querySelectorAll('[data-knowledge-file]')) {
    ...
    assert.deepEqual(
      ...
      readFileSync(new URL(`./assets/ai-knowledge/${name}`, import.meta.url)),
    );
  }
});
