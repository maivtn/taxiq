    ...
    return `
      <article class="billing-detail-summary" aria-labelledby="billing-summary-title">
        ...
        <dl class="billing-detail-meta">
          ...
          <div><dt>Seller</dt><dd>${escapeHTML(record.seller.legalName || record.seller.name)}${record.seller.phone ? `<span>${escapeHTML(record.seller.phone)}</span>` : ''}<span>${escapeHTML(record.seller.email)}</span></dd></div>
          <div data-billing-email-preview-hidden="true"><dt>Bill to</dt><dd>${escapeHTML(record.billTo.name)}<span>${escapeHTML(record.billTo.email)}</span></dd></div>
