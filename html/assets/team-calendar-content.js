...
function renderPolicyForm() {
  ...
  policyDrawer.innerHTML = `<div class="policy-head"><div><p class="eyebrow">OWNER ACCESS</p><h2>Booking incentive policy</h2><span>Reward completed appointment business and control its turn impact.</span></div><button class="close-policy" aria-label="Close reward settings">×</button></div>
    ...
    <section class="policy-section" id="weighted-turn-settings"><div class="policy-section-title"><span>↔</span><div><strong>Weighted Turn Settings</strong><small>Shared with Front Desk → Turn Board.</small></div></div><p class="shared-turn-note">Set service turn credits by service value after discounts, excluding tips, tax, products, and gift-card payments. Booking turn credit is configured above; technician overrides still apply.</p><div class="shared-turn-grid">${window.NEXORA_TURN_SETTINGS.labels.map((label, index) => `<label><span data-service-range-label="${index}">${label} · service turns</span><input data-service-weight="${index}" type="number" min="0" step="any" required value="${policy.serviceWeights[index]}"></label>`).join("")}</div><p class="shared-turn-note">Saving here updates Turn Board settings. Existing recorded turns stay unchanged. <a href="pos-front-desk-turn-board.html?settings=weighted-turns">Open Turn Board settings</a></p></section>
...
window.NEXORA_TURN_SETTINGS.subscribe(settings => {
  ...
  if (policyDrawer.classList.contains("is-open")) {
    ...
    policyDrawer.querySelectorAll("[data-service-weight]").forEach((input, index) => { input.value = settings.serviceWeights[index]; });
