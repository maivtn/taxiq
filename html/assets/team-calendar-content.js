...
const state = {
  ...
  rewardPolicy: { ...structuredClone(defaultRewardPolicy), turnCredit: window.NEXORA_TURN_SETTINGS.load().bookingTurnCredit },
  policyDraft: null,
...
function readPolicyForm() {
  const policy = structuredClone(state.policyDraft || state.rewardPolicy); policy.scope = policyDrawer.querySelector('[name="scope"]:checked')?.value || policy.scope; policy.structure = policyDrawer.querySelector("[data-structure].active")?.dataset.structure || policy.structure; policy.calculation = policyDrawer.querySelector('[name="calculation"]:checked')?.value || policy.calculation; policy.period = $("#reward-period")?.value || policy.period; policy.turnCredit = window.NEXORA_TURN_SETTINGS.numberFrom($("#turn-credit")); policy.flatRate = Number($("#flat-rate")?.value ?? policy.flatRate); policy.effectiveDate = $("#effective-date")?.value || policy.effectiveDate;
...
function validatePolicyTurns(policy) {
  const error = window.NEXORA_TURN_SETTINGS.validate({...window.NEXORA_TURN_SETTINGS.load(), bookingTurnCredit: policy.turnCredit});
...
function savePolicy() {
  ...
  const result = window.NEXORA_TURN_SETTINGS.save({...window.NEXORA_TURN_SETTINGS.load(), bookingTurnCredit: policy.turnCredit});
