(function () {
  ...
  function mount(root, options) {
    ...
    function billsHtml() {
      ...
      const toolsHtml=!parent.payment?`<div class="tw-bill-tools">${!amountSplit()&&group().bills.length<parent.lines.length?button('+ Add bill','data-tw-add-bill','tw-small'):''}${!amountSplit()&&group().bills.length>2&&!ticket.lines.length&&!ticket.payment?button('Remove empty bill','data-tw-remove-bill','tw-small'):''}${!paid?button('Cancel split','data-tw-cancel-split','tw-text tw-cancel-split'):''}</div>`:'';
      return `<section class="tw-card tw-bills"><div class="tw-bill-toolbar"><h3>Split bill <span class="tw-mode-badge">${amountSplit()?'By amount':'By services'}</span></h3><div class="tw-progress"><strong data-tw-bill-progress role="status">${paid}/${group().bills.length} bills paid</strong><progress value="${paid}" max="${group().bills.length}" aria-label="Bills paid"></progress><small>${money(sumPaid)} collected</small></div>${toolsHtml}</div><div class="tw-bill-tabs" role="group" aria-label="Guest bills">${group().bills.map((b,i)=>{
        ...
        return button(`<span class="tw-bill-card-top"><span class="tw-guest-avatar" aria-hidden="true">${i+1}</span><span class="tw-bill-person"><strong>Bill ${i+1} · ${esc(b.name)}</strong><small><span class="tw-bill-state ${b.payment?'is-paid':!lines.length?'needs-services':''}">${b.payment?'✓ ':''}${status}</span><span>${amountSplit()?'Shared ticket':lines.length+' service'+(lines.length===1?'':'s')}</span></small></span><strong class="tw-bill-amount" data-tw-bill-total="${esc(b.id)}">${t.error?'—':money(t.totalCents)}</strong></span><small class="tw-bill-services" title="${esc(lines.map(l=>l.name).join(' · '))}">${!amountSplit()?(lines.map(l=>esc(l.name)).join(' · ')||'Choose services below'):'A share of the full ticket total'}</small>`,`data-tw-bill="${b.id}" data-tw-guest-color="${guestColor(b)}" aria-pressed="${selected}"`,'tw-bill-card '+(selected?'selected':''));
      }).join('')}</div></section>`;
    }
    ...
    function paymentHtml() {
      ...
      <div class="tw-receipt-row"><h3 class="tw-receipt-label">Receipt</h3><div class="tw-receipt-options">${[['none','No Receipt'],['sms','Send SMS'],['print','Print']].map(([id,label])=>button(label,`data-tw-receipt="${id}"`,p.receipt===id?'selected':'')).join('')}</div><div class="tw-preview">${button('Print preview','data-tw-preview title="Print preview"','tw-text tw-icon-only')}</div></div></section>
      <section class="tw-card tw-payment-summary"><div class="tw-card-title"><h3>Payment summary</h3><div class="tw-ticket-tools">${button('Discount all','data-tw-discount-all'+(group()?.bills.some(b=>b.payment)?' disabled title="Discount is locked after the first bill is paid"':''),'tw-small tw-orange')}${!group()?button('Split bill','data-tw-split-bill aria-haspopup="dialog"','tw-small tw-purple'):''}</div></div><div class="tw-summary-breakdown"><div class="tw-summary-row"><span>Subtotal</span><span data-tw-subtotal></span></div><div class="tw-summary-row"><span>Tip</span><span data-tw-tip-total></span></div><div class="tw-summary-row"><span>Discount</span><span class="tw-red" data-tw-discount-total></span></div></div><div class="tw-summary-row tw-rule"><strong>Total</strong><strong data-tw-total></strong></div></section><p class="tw-demo">Prototype · Payment and SMS receipt are simulated.</p>`;
    }
    function render() {
      ...
      root.innerHTML=`<div class="tw-heading ${mode==='checkout'?'tw-checkout-heading':''}">${button('Back','data-tw-back')}<h2>Ticket #${esc(parent.id)} · ${esc(parent.customer)}</h2>${mode==='checkout'?'<span class="tw-checkout-label">Checkout</span>':''}${!paid&&!group()?button('Edit customer','data-tw-customer','tw-text'):''}</div>${billsHtml()}<div class="tw-workspace ${mode==='checkout'?'tw-checkout-layout':''}">${leftColumn}<div class="tw-ticket-side"><section class="tw-card"><div class="tw-card-title"><h3>${amountSplit()?'Shared ticket':'Ticket detail'} (${detailLines.length} services)</h3>${amountSplit()&&!group().bills.some(b=>b.payment)?button('Assign services','data-tw-assign-services aria-haspopup="dialog"','tw-small tw-purple'):''}${!paid&&!group()?`<div class="tw-ticket-tools">${mode==='checkout'?button('Add service','data-tw-add-service','tw-small tw-purple'):''}${button('Custom','data-tw-custom','tw-small tw-purple')}</div>`:''}</div>${amountSplit()?`<p class="tw-service-pick-note">This ticket is split by amount. ${group().bills.some(b=>b.payment)?'Each bill pays a share of the ticket total.':'Choose Assign services to select who pays for each service. Bill totals will follow the selected services.'}</p>`:''}<div class="tw-lines">${serviceSplit?serviceToolbarHtml():''}${detailLines.map(l=>{
        ...
        const selection=serviceSplit?`<label class="tw-service-pick"><input type="checkbox" data-tw-bill-line="${esc(l.id)}" aria-label="Assign ${esc(l.name)} to ${esc(ticket.customer)}" ${owner?.id===activeBillId?'checked':''} ${paid||owner?.payment?'disabled':''}><span class="tw-service-identity"><strong>${esc(l.name)}</strong><span class="tw-payer-badge" data-tw-guest-color="${owner?guestColor(owner):'none'}">${owner?'Payer: Bill '+guestNumber(owner)+' · '+esc(owner.name)+(owner.payment?' · Paid':''):'Not assigned'}</span></span></label>`:'';
