(function () {
  ...
  const statusKey = status => ({pending:'pendingCampaign',approved:'approvedCampaign',rejected:'rejectedCampaign'}[status] || status);
  function init(bridge) {
    ...
    $('#studio-results').innerHTML = '<header class="studio-section-heading"><div><p class="promo-eyebrow">Promotion Studio / Analytics</p><h2>'+tr('resultsTitle')+'</h2><p>'+tr('resultsHint')+'</p></div>'+select('reportPeriod','reportsPeriod',[['7','last7'],['30','last30']])+'</header><div class="studio-report-stats">'+['spend','held','adjustments'].map(key => '<article><span>'+tr(key)+'</span><strong>—</strong></article>').join('')+'</div><div class="studio-table-scroll"><table class="studio-table"><thead><tr>'+['channel','impressions','clicks','spend'].map(key => '<th scope="col">'+tr(key)+'</th>').join('')+'</tr></thead><tbody>'+['internal','organic','paid'].map(key => '<tr><th scope="row">'+tr(key)+'</th><td>—</td><td>—</td><td>—</td></tr>').join('')+'</tbody></table></div><p class="promo-note">'+tr('reportTime')+'</p><a class="promo-button" href="nexora-packages.html?tab=ads-credit" target="_blank" rel="noopener">'+tr('creditLink')+' ↗</a>';
  ...
  function tab(name) {
    activeTab = name;
    ['offers','campaigns','results'].forEach(key => { $('#studio-'+key).hidden = key !== name; const button = $('[data-studio-tab="'+key+'"]'); button.classList.toggle('active',key === name); button.setAttribute('aria-pressed',String(key === name)); });
  ...
  function refresh() {
    ...
    $('#campaign-list').innerHTML = visible.map(campaign => {
      ...
      const reason = campaign.status === 'ended' ? t('ended') : campaign.status === 'paused' ? t('paused') : (campaign.rejectionReason ? campaign.rejectionReason + ' ' : '') + blockers(offer).map(t).join(' ');
      return '<article class="studio-campaign-card"><div class="studio-campaign-header"><div><p class="promo-eyebrow">Paid Boost · CPC</p><h3>'+esc(campaign.name)+'</h3><p>'+esc(offer?.title || t('promotion'))+'</p></div><span class="promo-chip">'+esc(t(statusKey(campaign.status)))+'</span></div><dl class="studio-campaign-facts"><div><dt>'+esc(t('campaignStart'))+'</dt><dd>'+esc(campaign.startDate || '—')+' → '+esc(campaign.endDate || '—')+'</dd></div><div><dt>'+esc(t('dailyBudget'))+'</dt><dd>'+money(campaign.dailyBudget)+'</dd></div><div><dt>'+esc(t('totalBudget'))+'</dt><dd>'+money(campaign.totalBudget)+'</dd></div><div><dt>'+esc(t('spend'))+'</dt><dd>—</dd></div></dl><p class="studio-campaign-reason">'+esc(reason)+'</p><div class="promotion-actions">'+(campaign.status !== 'ended' ? button('edit',campaign.status === 'paused' ? 'resumeCampaign' : 'editCampaign')+(campaign.status === 'pending' ? button('pause','pauseCampaign') : '')+button('end','endCampaign') : button('view','viewCampaign'))+'</div></article>';
    }).join('');
  }
  ...
  function open(campaign) {
    ...
    $('#save-campaign').hidden = $('#submit-campaign').hidden = ended;
  ...
  function save(status) {
    ...
    if (!api.persist(next)) return error('saveError');
    $('#campaign-editor').close(); tab('campaigns'); api.feedback(t('campaignSaved'));
  }
  ...
  function validSaved(items) {
    return Array.isArray(items) && new Set(items.map(item => item?.id)).size === items.length && items.every(item => item &&
      ...
      ['draft','pending','paused','ended','approved','rejected'].includes(item.status) &&
