...
const STAFF_STYLE = readFileSync(new URL('./staff-shell.css', import.meta.url), 'utf8');
const ASSIGNMENTS_SOURCE = readFileSync(new URL('./service-assignments.js', import.meta.url), 'utf8');
...
test('staff sidebar links to My Tickets immediately above My Calendar and highlights only Tickets', () => {
  ...
  assert.ok(ticketsLink?.querySelector('[data-lucide="tickets"]'));
