...
Authority retry nằm trong `ui.pendingContext`, không nằm trong URL: `tipEntryIntent = scan | generic`, `tipScanReplayId` và fingerprint exact `{businessId, station, staffProfileId}`. State machine chỉ nhận prepared `{armed:true, replayId:null, tipId:null, fingerprint=current}` hoặc created/replay `{armed:false, replayId:tipId, fullBundle:valid}`. Double-send, reload, lỗi navigation hoặc quét lại cùng QR khi tip scan còn `pending` validate unique canonical record cùng amount/method/note rồi trả record đó với `idempotent: true`. Trước khi so incoming QR với prior QR, `stageSalonScan` validate state shape và trọn bundle; `createTipFromScan` direct retry dùng cùng pure lifecycle validator: pending có 0 linked tip ledger, confirmed có đúng 1 `isTipLedgerPair`. Partial/corrupt trả `invalid_tip_replay` trước khi đổi selector/context, phát UUID, mở payment hay navigation. Migration không đổi corrupt scan intent thành prepared/generic mà giữ quarantine qua reload; chỉ `prepareGenericTipContext` mới explicit chuyển mode. Pending hợp lệ được giữ khi cùng QR hoặc clear pointer khi sang QR canonical khác; chỉ terminal có ledger hợp lệ cho phép lượt mới.
