...
Authority retry nằm trong `ui.pendingContext`, không nằm trong URL: `tipEntryIntent = scan | generic`, `tipScanReplayId` và fingerprint exact `{businessId, station, staffProfileId}`. Double-send, reload, lỗi navigation hoặc quét lại cùng QR khi tip scan còn `pending` phải validate unique canonical record cùng amount/method/note rồi trả record đó với `idempotent: true`. Trước khi so incoming QR với prior QR, `stageSalonScan` validate trọn bundle: prior context phải exact/canonical; stored fingerprint phải bind context đó; replay ID phải trỏ duy nhất tới canonical tip và bằng `pendingContext.tipId`; tip target phải bằng prior business/staff; pure lifecycle validator yêu cầu pending có 0 linked tip ledger và confirmed có đúng 1 `isTipLedgerPair`. `none/corrupt` trả `invalid_tip_replay` trước khi thay `selectedBusinessId`, `scanContext` hay intent. Chỉ bundle `pending` hợp lệ được giữ khi cùng QR hoặc clear pointer khi sang QR canonical khác; chỉ bundle `terminal` có ledger hợp lệ cho phép lượt mới. Do đó sửa status/confirmedAt mà thiếu hoặc giả ledger không thể rửa replay. Generic tip phải qua `prepareGenericTipContext`.
