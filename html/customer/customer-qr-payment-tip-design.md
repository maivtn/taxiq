...
Authority retry nằm trong `ui.pendingContext`, không nằm trong URL: `tipEntryIntent = scan | generic`, `tipScanReplayId` và fingerprint exact `{businessId, station, staffProfileId}`. Double-send, reload, lỗi navigation hoặc quét lại cùng QR khi tip scan còn `pending` phải validate unique canonical record cùng amount/method/note rồi trả record đó với `idempotent: true`. Replay ID/context/record sai phải fail closed, không rơi xuống generic. Same/different QR phải so incoming QR với **prior `scanContext` được re-parse canonical**, tuyệt đối không dùng replay fingerprint có thể hỏng làm nguồn phân loại. Một QR canonical khác là explicit new scan intent: clear replay pointer của QR cũ, giữ transaction pending cũ làm history và tạo transaction mới chỉ cho exact target mới. Nếu prior context không resolve được, `stageSalonScan` từ chối atomic trước khi thay `selectedBusinessId`, `scanContext` hay intent; vì vậy chuỗi scan B → A không thể rửa replay hỏng. Cùng QR chỉ bắt đầu tip mới sau khi tip trước terminal; generic tip phải qua `prepareGenericTipContext`.
