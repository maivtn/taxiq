...
- Tip từ QR chỉ bật khi `staffProfileId` canonical có ít nhất một payment method; `prepareTipFromScan` và `createTipFromScan` re-parse payload để khóa đúng business/staff. `ui.pendingContext.tipEntryIntent = scan | generic`, `tipScanReplayId` và fingerprint business/station/staff giữ authority qua save/reload hoặc lỗi điều hướng. Scan intent chỉ có hai shape hợp lệ: **prepared** = `tipScanArmed=true`, replay ID `null`, fingerprint bind exact current context và `tipId=null`; **created/replay** = armed `false`, replay ID khác `null`, `tipId === replayId` và full canonical bundle hợp lệ. `prepareTipFromScan` phải xóa `tipId` cũ khi arm lượt fresh. Trước khi phân loại same/different QR, `stageSalonScan` validate toàn bộ intent/bundle; direct retry trong `createTipFromScan` cũng dùng lifecycle validator. Tip `pending` phải có 0 linked tip ledger; tip `confirmed` phải có đúng 1 ledger và `isTipLedgerPair` phải đúng. Mọi partial/corrupt combination trả `invalid_tip_replay` **trước mọi mutation**, không phát UUID hoặc mở payment; migration giữ corrupt scan intent ở quarantine qua reload, chỉ `prepareGenericTipContext` mới chuyển mode có chủ ý. Bundle pending hợp lệ + cùng QR re-enter record; pending hợp lệ + QR canonical khác nhường authority cho scan mới; chỉ terminal bundle có ledger hợp lệ cho phép lượt tip mới. URL không sở hữu replay ID hoặc transaction.
