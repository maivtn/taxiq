...
- Tip từ QR chỉ bật khi `staffProfileId` canonical có ít nhất một payment method; `prepareTipFromScan` và `createTipFromScan` re-parse payload để khóa đúng business/staff. `ui.pendingContext.tipEntryIntent = scan | generic`, `tipScanReplayId` và fingerprint business/station/staff giữ authority qua save/reload hoặc lỗi điều hướng: retry exact amount/method/note trả lại cùng tip `pending`, input/context/record sai thì fail closed. Trước khi phân loại same/different QR, `stageSalonScan` phải validate **toàn bộ prior replay bundle**: scan context exact/canonical, fingerprint binding, unique canonical tip, `tipScanReplayId === tipId`, target business/staff và lifecycle-ledger khớp. Tip `pending` phải có 0 linked tip ledger; tip `confirmed` phải có đúng 1 ledger và `isTipLedgerPair` phải đúng. Bundle `corrupt/none` bị từ chối atomic. Bundle pending hợp lệ + cùng QR re-enter record; pending hợp lệ + QR canonical khác là explicit new scan intent, xóa replay pointer cũ nhưng giữ tip cũ trong lịch sử. Chỉ bundle terminal có ledger hợp lệ mới cho phép lượt tip mới. Mọi inconsistency trả `invalid_tip_replay` **trước mọi mutation**. URL không sở hữu replay ID hoặc transaction.
