...
- Tip: chỉ bật khi QR có staff canonical và staff đã bật ít nhất một phương thức; helper re-parse QR khi prefill và khi tạo tip để không tin selector DOM. `tipEntryIntent`, `tipScanReplayId` và fingerprint context được persist. Prepared scan phải có armed `true`, replay ID/`tipId` cùng `null` và fingerprint bind exact context; created/replay phải có armed `false`, replay ID bằng `tipId` và full bundle hợp lệ. Stage, prepare và direct create retry đều fail closed trước mutation nếu shape/context/target/lifecycle-ledger sai. Pending phải không có linked ledger; confirmed phải có đúng một canonical `isTipLedgerPair`. Corrupt scan intent vẫn bị quarantine sau reload và chỉ explicit generic switch mới xóa authority. Chỉ pending bundle hợp lệ được reuse cùng QR hoặc nhường authority cho QR canonical khác; chỉ terminal bundle có ledger hợp lệ cho phép lượt mới. URL không mang replay authority.
