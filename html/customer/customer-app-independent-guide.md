...
- Tip: chỉ bật khi QR có staff canonical và staff đã bật ít nhất một phương thức; helper re-parse QR khi prefill và khi tạo tip để không tin selector DOM. `tipEntryIntent`, `tipScanReplayId` và fingerprint context được persist: double-send, reload, lỗi điều hướng hoặc quét lại cùng QR trong lúc tip còn pending đều reuse đúng transaction; context/input/record sai fail closed. Same/different được xác định từ scan context cũ parse lại canonical, không từ replay fingerprint. Một QR canonical khác xóa replay authority cũ, giữ tip pending cũ làm lịch sử và bắt đầu exact scan intent cho recipient mới. Nếu context cũ không resolve được, mọi rescan trả `invalid_tip_replay` trước mutation để không thể dùng chuỗi QR B → A rửa authority. Cùng QR chỉ tạo tip mới sau terminal; URL không mang replay authority.
