...
- Tip: chỉ bật khi QR có staff canonical và staff đã bật ít nhất một phương thức; helper re-parse QR khi prefill và khi tạo tip để không tin selector DOM. `tipEntryIntent`, `tipScanReplayId` và fingerprint context được persist: double-send, reload, lỗi điều hướng hoặc quét lại cùng QR trong lúc tip còn pending đều reuse đúng transaction. Trước same/different, app validate atomic prior context exact, fingerprint binding, unique canonical tip, `tipId`, target và tip-ledger lifecycle. Pending phải không có linked ledger; confirmed phải có đúng một canonical `isTipLedgerPair`. Bundle hỏng trả `invalid_tip_replay` trước mutation. Chỉ pending bundle hợp lệ mới được reuse cùng QR hoặc nhường authority cho QR canonical khác; chỉ terminal bundle có ledger hợp lệ cho phép lượt mới. URL không mang replay authority.
